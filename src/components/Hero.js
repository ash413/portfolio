export default function Hero() {
  return (
    <section id="hero" className="min-h-screen flex flex-col justify-center px-6 md:px-12 pt-28 pb-16 max-w-5xl mx-auto relative">
      ...
      <h1 className="text-[clamp(40px,8vw,96px)] font-light text-[var(--text)] leading-none tracking-tight mb-3">
        Vaishnavi<br />Kadam
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-10">
        <a href="#projects" className="font-mono text-xs bg-green-500 text-[#030a05] px-6 py-2.5 rounded-sm font-medium tracking-wider hover:opacity-85 transition-opacity text-center">
          View my work
        </a>
        <a href="#contact" className="font-mono text-xs text-[var(--text-muted)] border border-[var(--border-strong)] px-6 py-2.5 rounded-sm tracking-wider hover:border-[var(--text-dim)] hover:text-[var(--text)] transition-all text-center">
          Get in touch
        </a>
      </div>
      ...
      <div className="hidden md:flex absolute bottom-9 left-12 items-center gap-2.5">
        ...
        <span className="font-mono text-[10px] text-[var(--text-dim)] uppercase tracking-widest">Scroll</span>
      </div>
    </section>
  );
}